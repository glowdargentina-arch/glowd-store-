...
(() => {
  ...
  const subs = new Set();
  let slots = {};
  // Public read API so other code on the page can read the in-memory slot
  // store without re-fetching the sidecar (which the dev preview can 403).
  // Returns a shallow snapshot; consumers should call again after a change
  // (subscribe via the `image-slots-change` window event).
  window.imageSlotsSnapshot = () => Object.assign({}, slots);
  // Kick off the sidecar load eagerly (don't wait for a slot to connect),
  // so consumers reading the snapshot in view-only mode get data too.
  window.imageSlotsLoad = () => load();
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        // Fallback: if the sidecar can't be fetched (dev preview can 403),
        // use a baked-in JS literal populated by image-slots-data.js.
        if ((!j || typeof j !== 'object') && window.IMAGE_SLOTS_DATA && typeof window.IMAGE_SLOTS_DATA === 'object') {
          j = window.IMAGE_SLOTS_DATA;
        }
      ...
      .catch(() => {})
      .then(() => { loaded = true; subs.forEach((fn) => fn()); try { window.dispatchEvent(new CustomEvent('image-slots-change', { detail: { hydrated: true } })); } catch (e) {} });
    return loadP;
  }
  ...
  function setSlot(id, val) {
    ...
    subs.forEach((fn) => fn());
    try { window.dispatchEvent(new CustomEvent('image-slots-change', { detail: { id } })); } catch (e) {}
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        // Honor an explicit encode-width attribute (in px) so authors can
        // request a hi-res encode for slots that are small on-page but get
        // upscaled later (e.g. a 100px avatar that opens fullscreen).
        const attrEncode = parseInt(this.getAttribute('encode-width') || '', 10);
        const w = (attrEncode > 0 ? attrEncode : (this.clientWidth || this.offsetWidth)) || MAX_DIM;
